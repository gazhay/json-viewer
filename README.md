...
| `expanded`                            | Number or Boolean              | 1               | The initial depth to which the JSON data is expanded. You can also set it to `true` to fully expand the data, or `false` to collapse it.                                              |
...
| `expandIconType` (`expand-icon-type`) | String                         | "arrow"         | The type of icons used for expanding and collapsing JSON nodes. You can choose "square", "circle" or "arrow".                                                                         |
| `showCopy` (`show-copy`)              | Boolean                        | true            | Whether to show the copy button to copy the JSON data to the clipboard.                                                                                                               |
| `copyWithKey` (`copy-with-key`)       | Boolean                        | false           | Whether to copy both key and value (`"key": value`) or just the value when clicking the copy button. When `false` (default), only the value is copied for backward compatibility.    |
